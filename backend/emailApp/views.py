...
def alertPDF(request, formId):
    data = RiskFactors.objects.filter(id=formId).values().first()
    ...
    data['val4n'] = val4n
    data['user'] = UserAccount.objects.filter(id=19).values().first()
    data['advisor'] = UserAccount.objects.filter(id=19).values().first()
    context = data
    template = get_template('alertPDF.html')
    cmd_options = {
      'page-size': 'Letter',
...
def sendEmail(request):
    ...
    template = get_template('alertEmail.html').render(context=data)
...
def sendAgentAlertEmail(request, formId):
    
    data = {}
    data['protocol'] = 'http'
    userId = RiskFactors.objects.filter(id=formId).values("advisorId").first()['advisorId']
    data['formData'] = RiskFactors.objects.filter(id=formId).values().first()
    data['user'] = UserAccount.objects.filter(id=userId).values().first()
    userEmail = data['user']['email']
    template = get_template('alertEmailAgent.html').render(context=data)
    # print("Hi "+user[0]['email'])
    msg = EmailMultiAlternatives(
        'HIGH DRA recorded on of %s' %(datetime.today().strftime('%d %b %Y')),
        None, # This is the text context, just send None or Send a string message
    ...
    print({'message': "Email Sent"}, 200)

def sendAlertEmail(request, formId, advisorId):
    ...
    data['URL'] = env('FRONTEND_URL')  +"/alertForm?userId=" + userId + "&formId=" + formIdEncoded
    template = get_template('alertEmail.html').render(context=data)
    # print("Hi "+user[0]['email'])
    msg = EmailMultiAlternatives(
        'HIGH DRA recorded on of %s' %(datetime.today().strftime('%d %b %Y')),
    ...
    msg.attach_file(fileName)
    msg.send(fail_silently=False)
    sendAgentAlertEmail(request=request, formId=formId)
    print({'message': "Email Sent"}, 200)

def approveDenyEmail(request, adminId, user, formData, status):
    
    data = {}
    data['protocol'] = 'http'
    data['admin'] = UserAccount.objects.filter(id=adminId).values().first()
    data['user'] = user
    data['formData'] = formData
    data['status'] = status
    template = get_template('approveDenyEmail.html').render(context=data)
    # print("Hi "+user[0]['email'])
    msg = EmailMultiAlternatives(
        'SFP Form %s for Client %s at %s' %(status,data['formData']['RF_ClientName'],datetime.today().strftime('%d %b %Y')),
        None, # This is the text context, just send None or Send a string message
        'SFP ROA',
        [user['email']],
    )
...
class PasswordResetEmail(BaseEmailMessage):
    ...
    def get_context_data(self):
        ...
        context["URL"] = env('FRONTEND_URL') +"/reset-password-confirm"
